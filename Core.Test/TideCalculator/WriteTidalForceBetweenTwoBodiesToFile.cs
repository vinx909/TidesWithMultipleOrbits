...
{
    public class WriteTidalForceBetweenTwoBodiesToFile : TideCalculatorTestBase
    {
        [Fact]
        public void ReturnsFalse_WhenSetWritePathNotSet()
        {
            ...
            var result = sut.WriteTidalForceBetweenTwoBodiesToFile(sataliteOne, sataliteTwo, 0, 3, 1);
        ...
        public void InvokesIWriterCanWrite()
        {
            ...
            sut.WriteTidalForceBetweenTwoBodiesToFile(sataliteOne, sataliteTwo, 0, 3, 1);
        ...
        public void InvokesIWriterWriteWithCorrectStrings_StartWithLeadingZeros()
        {
            ...
            var result = sut.WriteTidalForceBetweenTwoBodiesToFile(sataliteOne, sataliteTwo, 0, 10, 5);
        ...
        public void InvokesIWriterWriteWithCorrectStrings_value()
        {
            ...
            var sataliteOne = SystemOneSataliteOne;
            var sataliteTwo = SystemOneSataliteTwo;
            double expectedOne = GetTidalForce(sataliteTwo.Mass, sataliteOne.Radius, sataliteOne.OrbitingDistance - sataliteTwo.OrbitingDistance);
            double expectedTwo = GetTidalForce(sataliteTwo.Mass, sataliteOne.Radius, Math.Pow(Math.Pow(sataliteOne.OrbitingDistance, 2) + Math.Pow(sataliteTwo.OrbitingDistance, 2), 0.5));
            double expectedThree = GetTidalForce(sataliteTwo.Mass, sataliteOne.Radius, sataliteOne.OrbitingDistance + sataliteTwo.OrbitingDistance);
            double expectedFour = GetTidalForce(sataliteTwo.Mass, sataliteOne.Radius, Math.Pow(Math.Pow(sataliteOne.OrbitingDistance, 2) + Math.Pow(sataliteTwo.OrbitingDistance, 2), 0.5));
            mockWriter.Setup(m => m.CanWriteTo(path)).Returns(true);
            mockWriter.Setup(m => m.IsWriting()).Returns(false);
            mockWriter.Setup(m => m.StartWriting(path)).Returns(true);
            mockWriter.Setup(m => m.Write(It.IsAny<string>())).Returns(true);
